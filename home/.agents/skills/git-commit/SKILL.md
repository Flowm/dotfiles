...
1. The **message** follows the Conventional Commits shape: `type(scope): description`.
2. The **footer** carries an `Assisted-by:` trailer naming the AI agent and model that helped — disclosure per the Linux kernel's AI Coding Assistants policy, so `git log`/`git blame` stay an honest record.
